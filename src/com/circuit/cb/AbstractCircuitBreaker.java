...
public abstract class AbstractCircuitBreaker implements CircuitBreaker {
    ...
    //在熔断器关闭的情况下，在多少秒内失败多少次进入打开状态（默认1分钟内，失败10次进入打开状态）
    public String thresholdFailRateForClose = "10/60";

    //在熔断器打开的情况下，熔断多少秒进入半开状态，（默认熔断3分钟）
    public int thresholdIdleTimeForOpen = 180;

    //在熔断器半开的情况下, 在多少秒内放多少次请求，去试探(默认1分钟内，放10次请求)
    public String thresholdPassRateForHalfOpen = "10/60";
    ...
    public void setState(CBState state) {
        ...
        if (currentState.getStateName().equals(state.getStateName())){//一个状态不能切换为它自己
            return;
        }
        //多线程环境加锁
        synchronized (this){
            ...
            // 更新状态
            this.state = state;
            System.out.println("状态转移：" + currentState.getStateName() + "->" + state.getStateName());
        }
    }
}